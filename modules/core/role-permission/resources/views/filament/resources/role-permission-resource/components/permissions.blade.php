...
<div class="space-y-6"
     ...
     x-init="$nextTick(() => updateGlobal())"
     @change.capture="updateGlobal()">

    <!-- Global Stats Header -->
    <div class="flex items-center justify-between bg-white dark:bg-gray-900 px-4 py-3 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm mb-6">
        <h3 class="font-bold text-gray-700 dark:text-gray-200">{{ __('Total Permissions') }}</h3>
        <div class="flex items-center gap-3">
             <button type="button" 
                    @click="reorderMode = !reorderMode; $dispatch('toggle-sortable', reorderMode)"
                    :class="reorderMode ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300'"
            ...
            <span class="text-sm font-bold px-3 py-1 rounded-full bg-primary-50 dark:bg-primary-900/50 text-primary-600 dark:text-primary-400 border border-primary-100 dark:border-primary-800">
    ...
    @foreach ($groupedPermissions as $module => $groups)
        ...
        <div class="mb-4" 
             ...
             x-init="$nextTick(() => updateModuleCount())"
             @change.capture="updateModuleCount()"
             wire:key="module-{{ $currentModuleIndex }}">
            <div @click="open = !open" 
                 class="flex items-center justify-between cursor-pointer bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 transition relative">
                
                <!-- Module Drag Handle (Overlay) -->
                <div x-show="reorderMode" class="drag-handle absolute inset-0 z-10 cursor-move bg-white/50 dark:bg-black/50 flex items-center justify-center rounded-lg border-2 border-dashed border-gray-400 dark:border-gray-500">
                    <span class="bg-white dark:bg-gray-800 px-3 py-1 rounded shadow text-xs font-bold uppercase tracking-wider">{{ __('Drag to Reorder') }}</span>
                </div>
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300">
            ...
            <div x-show="open" x-collapse
                ...
                @foreach ($groups as $group => $permissions)
                    ...
                    @click.outside="groupExpanded = false"
                    id="{{ $groupId }}"
                    ...
                    class="relative bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 hover:shadow-md transition-shadow duration-300 h-fit self-start">
                        ...
                        <div x-on:click.stop="groupExpanded = !groupExpanded"
                             class="bg-gray-50 dark:bg-gray-800 rounded-xl px-5 py-3 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center cursor-pointer relative">
                             
                            <!-- Group Drag Handle -->
                            <div x-show="reorderMode" class="drag-handle absolute inset-0 z-10 cursor-move bg-white/50 dark:bg-black/50 flex items-center justify-center rounded-xl border border-dashed border-gray-300">
                                <!-- Reusing same label style or making it icon only -->
                                <svg class="w-6 h-6 text-gray-700 dark:text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
                            </div>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs font-medium px-1.5 py-0.5 rounded bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
                        ...
                        <div x-show="groupExpanded" 
                            ...
                            @foreach ($permissions as $permission)
                                <div class="flex items-center justify-between group relative" data-sort-id="{{ $permission->id }}">
                                    <!-- Permission Drag Handle -->
                                    <div x-show="reorderMode" class="drag-handle absolute inset-y-0 -left-2 w-8 z-10 cursor-move flex items-center justify-center text-gray-400 hover:text-gray-600">
