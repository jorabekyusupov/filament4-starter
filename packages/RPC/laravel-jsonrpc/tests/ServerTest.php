...
class ServerTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testSingleRequestWithExceptionHandler()
    {
        ...
        $server->onException(
            ...
            function (\Exception $e, jsonrpc\src\Server\Request $request) {
                return jsonrpc\src\Server\RequestResponse::constructErrorResponse($request->getId(), $e->getMessage(), 403);
            }
        );
    ...
    public function testThrowableFromMiddleware()
    {
        ...
        $server->onException(
            Throwable::class,
            function (\Throwable $e, ?jsonrpc\src\Server\Request $request = null) use (&$throwable) {
                $throwable = $e;
                return jsonrpc\src\Server\RequestResponse::constructErrorResponse($request ? $request->getId() : null, $e->getMessage(), $e->getCode());
            }
        );
    ...
    public function testExceptionHandlersPriority()
    {
        ...
        $server->onException(
            \Exception::class,
            function (\Exception $_, jsonrpc\src\Server\Request $request) {
                return jsonrpc\src\Server\RequestResponse::constructErrorResponse($request->getId(), 'Error', -32999);
            }
        );
    ...
    private function assembleServer()
    {
        return new Server\Server(
            new jsonrpc\src\Server\RequestFactory(),
            new Server\Router(),
            new jsonrpc\src\Server\RouteDispatcher($this->constructContainerMock()),
            new jsonrpc\src\Server\MiddlewarePipelineDispatcher($this->constructContainerMock()),
